...
- `edit` | github-pages-deployment + local-only-overrides — Removed `/public/sprites/` from `.gitignore` and committed 58 previously-untracked sprite PNGs (commit `52dbc26`, master). The ignore rule meant locally-generated sprites never reached the Pages deploy → 404s on the live site. `local-only-overrides` no longer lists `.gitignore` (now matches HEAD); `github-pages-deployment` got a new "Sprites are committed to the repo" section + orphan note for `/sprites/10061.png`.
- `edit` | github-pages-deployment — `metadata.icons` is NOT auto-prefixed by Next under `basePath`; `layout.tsx` now inlines `NEXT_PUBLIC_BASE_PATH` for `icon-192/512.png` + `apple-touch-icon.png`. Verified emitted `<link rel="icon" href="/ChampionsLab/...">` in `out/index.html`. Wiki page corrected (the old "icons handled automatically" claim was wrong); openGraph/twitter images left bare because `metadataBase` resolves them to absolute URLs.
